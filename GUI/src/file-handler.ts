...
const initFileHandler = (scene: Scene, events: Events, dropTarget: HTMLElement) => {
    ...
    const importSplatModel = async (files: ImportFile[], animationFrame: boolean) => {
        try {
            ...
            // Diagnostic logging: inspect GSplatData and vertex properties (helps debug missing gaussians)
            try {
                ...
                if (gsplatData) {
                    ...
                    try {
                        ...
                        if (vertex && vertex.properties) {
                            ...
                            const sample: any = {};
                            vertex.properties.forEach((p: any) => {
                                try {
                                    const storage = p.storage;
                                    if (storage && storage.length) {
                                        sample[p.name] = Array.from(storage.slice(0, Math.min(5, storage.length)));
                                    }
                                } catch (e) {
            ...
            // Protect scene.add with try/catch to capture runtime errors and display a popup
            try {
