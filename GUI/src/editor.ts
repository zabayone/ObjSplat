...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    [
        ...
        'selection.changed', 'tool.coordSpace', 'objectPanel.glow',
        'splat.tintClr', 'splat.temperature', 'splat.saturation', 'splat.brightness',
        'splat.blackPoint', 'splat.whitePoint', 'splat.transparency'
    ].forEach((eventName) => {
        events.on(eventName, () => {
    ...
    const setObjectMode = (value: boolean) => {
        const nextValue = !!value;
        if (nextValue !== objectMode) {
            objectMode = nextValue;
            events.fire('view.objectMode', objectMode);
        }
    };

    events.function('view.objectMode', () => {
        return objectMode;
    });

    events.on('view.setObjectMode', (value: boolean) => {
        setObjectMode(value);
    });
    ...
    events.function('select.point', async (op: 'add'|'remove'|'set', point: { x: number, y: number }) => {
        const { width, height } = scene.targetSize;
        const mode = events.invoke('camera.mode');
        const objectMode = !!events.invoke('view.objectMode');

        if (objectMode) {
            await events.invoke('select.objectPick', op, point);
            return;
        }
    ...
    events.function('select.objectPick', async (op: 'add'|'remove'|'set', point: { x: number, y: number }) => {
        const splats = selectedSplats();
        const targetSize = scene.targetSize;
        if (!splats.length || !targetSize || !point) {
            return;
        ...
        for (const splat of splats) {
            ...
            const pickBuffer = await scene.camera.pickRect(nx, ny, 1 / width, 1 / height);
            const pickId = pickBuffer?.[0];
            if (pickId === undefined || pickId === 0xffffffff || pickId < 0 || pickId >= labels.length) {
                continue;
            }

            const label = Number(labels[pickId]);
            if (!Number.isFinite(label) || label <= 0) {
                continue;
            }

            const filter = (i: number) => Number(labels[i]) === label;
            events.fire('edit.add', new SelectOp(splat, op, filter));
        }
    });
