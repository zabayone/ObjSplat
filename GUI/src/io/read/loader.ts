...
// Sorting very large datasets can temporarily duplicate large buffers and stall the UI.
const MAX_ROWS_FOR_MORTON_REORDER = 1_000_000;
...
const dataTableToGSplatData = (dataTable: DataTable): GSplatData => {
    ...
    console.log('DataTable columns:', dataTable.columns.map((col: Column) => ({
        ...
        storage: col.data?.constructor?.name,
        length: col.data?.length,
        sample: col.data ? Array.from(col.data.slice(0, Math.min(5, col.data.length))) : []
    })));
...
const loadGSplatData = async (filename: string, fileSystem: ReadFileSystem, skipReorder?: boolean): Promise<GSplatData> => {
    ...
    if (inputFormat !== 'sog' && !isCompressedPly && !skipReorder && tables[0].numRows <= MAX_ROWS_FOR_MORTON_REORDER) {
