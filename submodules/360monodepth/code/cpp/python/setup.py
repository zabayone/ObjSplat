...
dist_dll_ext = "*.so*"  # Default, will be overridden

if is_macos:
    # macOS: CPU-only fallback, skip C++ extensions
    print("macOS detected: skipping C++ extensions, using CPU-only fallback")
...
if len(dll_filepath_list) == 0:
    print("Warning: Do not find dependent dynamic library (CPU-only mode)")

# clean & copy dynamic link libs (skip on macOS)
if not is_macos:
