...
# EigenSolvers module not available on macOS ARM64 - use scipy fallback
...
class BlendIt:
    def __init__(self, padding, n_subimages, blending_method):
        ...
        if blending_method == "all" or blending_method == "poisson":
            ...
            # EigenSolvers not available on macOS - use scipy fallback instead
            self.eigen_solver = None  # LinearSolver(LinearSolver.solverType.BiCGSTAB)
        else:
