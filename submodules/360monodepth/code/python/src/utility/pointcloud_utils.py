...
def depthmap2pointcloud_erp(depth_map, rgb_image, output_ply_file_path, label_map=None, point_labels=None):
    ...
    :param label_map: Optional per-pixel object label map aligned with the ERP image.
    :type label_map: numpy
    :param point_labels: Optional per-point labels aligned with the flattened ERP raster order.
    :type point_labels: numpy
    """
    ...
    if point_labels is not None:
        ...
        if labels.shape[0] != x.shape[0]:
            raise ValueError("point_labels must have the same number of elements as depth_map pixels")
    elif label_map is not None:
        label_map = np.asarray(label_map, dtype=np.int32)
        if label_map.shape != depth_map.shape:
            raise ValueError("label_map must have the same shape as depth_map")
    ...
    if labels is not None:
        point_cloud_data = np.stack([x, y, z, r, g, b, labels], axis=1)
    else:
    ...
    if labels is not None:
        fid.write(bytes('property int label\n', 'utf-8'))
    fid.write(bytes('end_header\n', 'utf-8'))

    # Write 3D points to .ply file
    for i in range(point_cloud_data.shape[0]):
        if labels is not None:
            fid.write(bytearray(struct.pack(
                "<fffcccI",
