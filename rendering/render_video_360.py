...
# Seleziona solo file nel formato esatto gsplat_layer<id>.ply
layer_pattern = re.compile(r"^gsplat_layer(\d+)\.ply$")
layer_ids = []
...
if not layer_ids:
    raise FileNotFoundError(f"Nessun file gsplat_layer<id>.ply trovato in {save_dir}")

layer_ids.sort()
print("layer_ids:", layer_ids)
layer_name = f"layer{layer_ids[-1]}"
...
# Usa device disponibile: MPS se su macOS, CUDA se disponibile, altrimenti CPU
