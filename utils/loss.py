...
with torch.no_grad():
    ...
    # Device will be set dynamically based on input tensor
...
def nearMean_map(array, mask, kernelsize=3):
    ...
    # Move conv to same device as input array
    conv_device = conv.to(array.device)
    nearMean_map = conv_device((array * mask)[None,None])
    cnt_map = conv_device((cnt_map * mask)[None,None])
