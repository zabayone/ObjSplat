...
try:
    ...
    DEPTHMAP_ALIGN_AVAILABLE = True
except (ModuleNotFoundError, ImportError):
    log.warn("depthmapAlign python module not available (C++ extension unavailable on macOS). Using software-only alignment fallback.")
    depthmapAlign = None
    DEPTHMAP_ALIGN_AVAILABLE = False
...
class DepthmapAlign:
    ...
    def __init__(self, opt, output_dir, subimages_rgb, debug=False):
        ...
        if DEPTHMAP_ALIGN_AVAILABLE:
            ...
            log.warn("depthmapAlign initialization skipped - using pure Python fallback")
    ...
    def align_single_res(self, depthmap_original_list, pixels_corresponding_list):
        ...
        try:
            if DEPTHMAP_ALIGN_AVAILABLE:
                ...
                # Fallback: use original depth maps without C++ alignment
                log.warn("Using fallback depth map alignment (pure Python, no Ceres optimization)")
                self.depthmap_aligned = depthmap_original_list
                # Create identity coefficients for all depth maps (scale=1, offset=0)
                align_coeff = []
                for i in range(self.depthmap_number):
                    align_coeff.append(np.ones((self.align_coeff_grid_height, self.align_coeff_grid_width), dtype=np.float64))  # scale
                    align_coeff.append(np.zeros((self.align_coeff_grid_height, self.align_coeff_grid_width), dtype=np.float64))  # offset
