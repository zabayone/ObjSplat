...
# Helper per selezionare il device corretto (MPS > CUDA > CPU)
def get_device():
...
class LayerGaussian:
    ...
    def capture_total(self):
        ...
        xyz_gradient_accum_static =  torch.zeros((self._xyz_static.shape[0], 1), device=get_device())
        xyz_gradient_accum = torch.cat([xyz_gradient_accum_static, self.xyz_gradient_accum], dim=0)
        max_radii2D_static = torch.zeros((self._xyz_static.shape[0]), device=get_device())
        max_radii2D = torch.cat([max_radii2D_static, self.max_radii2D], dim=0)
        denom_static = torch.zeros((self._xyz_static.shape[0], 1), device=get_device())
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        points_cpu = torch.tensor(np.asarray(pcd.points)).float()
        fused_point_cloud = points_cpu.to(device)
        ...
        dist2 = torch.clamp_min(distCUDA2(fused_point_cloud.detach()), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device=get_device())
        rots[:, 0] = 1

        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device=get_device()))
    ...
    def create_from_pcd_and_masks(self, pcd : BasicPointCloud, masks : BasicPointCloud, spatial_lr_scale : float):
        self.spatial_lr_scale = spatial_lr_scale
        device = get_device()
        identity = torch.tensor(np.asarray(masks.colors)[:,0]).float().to(device)
        ...
        # Keep masks on CPU and convert to numpy for indexing operations
        identity_mask_np = identity_mask.detach().cpu().numpy() if isinstance(identity_mask, torch.Tensor) else identity_mask
        outlier_mask_np = outlier_mask if isinstance(outlier_mask, np.ndarray) else outlier_mask.detach().cpu().numpy()
        
        # Use tensor versions for torch operations
        identity_mask_cpu = torch.from_numpy(identity_mask_np)
        outlier_mask_cpu = torch.from_numpy(outlier_mask_np)
        
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors), dtype=torch.float32)[identity_mask_np][outlier_mask_np].to(device))
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().to(device)
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0
        ...
        xyz_part = np.asarray(pcd.points)[identity_mask_np][outlier_mask_np]
        colors_part = np.asarray(pcd.colors)[identity_mask_np][outlier_mask_np]

        # xyz_part = np.asarray(pcd.points)[identity_mask_np][outlier_mask_np][outlier_mask2.detach().cpu().numpy()]
        # colors_part = np.asarray(pcd.colors)[identity_mask_np][outlier_mask_np][outlier_mask2.detach().cpu().numpy()]
        ...
        return identity[identity_mask][outlier_mask], _xyz, _features_dc, _features_rest, _scaling, _rotation, _opacity, identity_mask_np, outlier_mask_np      
    ...
    def load_ply_and_create_pcd(self, plydata, pcd, masks, spatial_lr_scale):
        ...
        if plydata is not None:
            ...
            _xyz = torch.cat([_xyz, torch.tensor(xyz2[active_mask], dtype=torch.float, device=get_device())], dim=0)
            _features_dc = torch.cat([_features_dc, torch.tensor(features_dc[active_mask], dtype=torch.float, device=get_device()).contiguous()], dim=0)
            _features_rest = torch.cat([_features_rest, torch.tensor(features_extra[active_mask], dtype=torch.float, device=get_device()).contiguous()], dim=0)
            _scaling = torch.cat([_scaling, torch.tensor(scales[active_mask], dtype=torch.float, device=get_device())], dim=0)
            _rotation = torch.cat([_rotation, torch.tensor(rots[active_mask], dtype=torch.float, device=get_device())], dim=0)
            # opacities[active_mask] = -3
            
            _opacity = torch.cat([_opacity, torch.tensor(opacities[active_mask], dtype=torch.float, device=get_device())], dim=0)
            identity_active = torch.ones((xyz[active_mask].shape[0]), dtype=torch.float, device=get_device())
            identity_active[:] = 2.0
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device=get_device())
        ...
        if plydata is not None:
            ...
            self._xyz_static = nn.Parameter(torch.tensor(xyz2[static_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
            self._features_dc_static = nn.Parameter(torch.tensor(features_dc[static_mask], dtype=torch.float, device=get_device()).contiguous().requires_grad_(True))
            self._features_rest_static = nn.Parameter(torch.tensor(features_extra[static_mask], dtype=torch.float, device=get_device()).contiguous().requires_grad_(True))
            self._opacity_static = nn.Parameter(torch.tensor(opacities[static_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
            self._scaling_static = nn.Parameter(torch.tensor(scales[static_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
            self._rotation_static = nn.Parameter(torch.tensor(rots[static_mask], dtype=torch.float, device=get_device()).requires_grad_(True))

            self._xyz_surface = nn.Parameter(torch.tensor(xyz2[surface_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
            self._features_dc_surface = nn.Parameter(torch.tensor(features_dc[surface_mask], dtype=torch.float, device=get_device()).contiguous().requires_grad_(True))
            self._features_rest_surface = nn.Parameter(torch.tensor(features_extra[surface_mask], dtype=torch.float, device=get_device()).contiguous().requires_grad_(True))
            self._opacity_surface = nn.Parameter(torch.tensor(opacities[surface_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
            self._scaling_surface = nn.Parameter(torch.tensor(scales[surface_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
            self._rotation_surface = nn.Parameter(torch.tensor(rots[surface_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
            self.identity_surface = torch.zeros((xyz2[surface_mask].shape[0]), device=get_device())
        ...
        #     _xyz_new = torch.cat([_xyz, torch.tensor(xyz2[surface_mask], dtype=torch.float, device=get_device())], dim=0)
        #     _features_dc_new = torch.cat([_features_dc, torch.tensor(features_dc[surface_mask], dtype=torch.float, device=get_device())], dim=0)
        #     _features_rest_new = torch.cat([_features_rest, torch.tensor(features_extra[surface_mask], dtype=torch.float, device=get_device())], dim=0)
        #     _scaling_new = torch.cat([_scaling, torch.tensor(scales[surface_mask], dtype=torch.float, device=get_device())], dim=0)
        #     _rotation_new = torch.cat([_rotation, torch.tensor(rots[surface_mask], dtype=torch.float, device=get_device())], dim=0)
        #     _opacity_new = torch.cat([_opacity, torch.tensor(opacities[surface_mask], dtype=torch.float, device=get_device())], dim=0)
        ...
        #     self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device=get_device())
        #     identity_new = torch.ones((xyz2[surface_mask].shape[0]), dtype=torch.float, device=get_device())
    ...
    def load_gaussians(self, plydata, spatial_lr_scale):
        ...
        _xyz = torch.tensor(xyz[active_mask], dtype=torch.float, device=get_device())
        _features_dc = torch.tensor(features_dc[active_mask], dtype=torch.float, device=get_device()).contiguous()
        _features_rest = torch.tensor(features_extra[active_mask], dtype=torch.float, device=get_device()).contiguous()
        _scaling = torch.tensor(scales[active_mask], dtype=torch.float, device=get_device())
        _rotation = torch.tensor(rots[active_mask], dtype=torch.float, device=get_device())
        # opacities[active_mask] = -3
        
        _opacity = torch.tensor(opacities[active_mask], dtype=torch.float, device=get_device())
        identity_active = torch.ones((xyz[active_mask].shape[0]), dtype=torch.float, device=get_device())
        identity_active[:] = 2.0
        ...
        self._xyz_static = nn.Parameter(torch.tensor(xyz[static_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
        self._features_dc_static = nn.Parameter(torch.tensor(features_dc[static_mask], dtype=torch.float, device=get_device()).contiguous().requires_grad_(True))
        self._features_rest_static = nn.Parameter(torch.tensor(features_extra[static_mask], dtype=torch.float, device=get_device()).contiguous().requires_grad_(True))
        self._opacity_static = nn.Parameter(torch.tensor(opacities[static_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
        self._scaling_static = nn.Parameter(torch.tensor(scales[static_mask], dtype=torch.float, device=get_device()).requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device=get_device())
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device=get_device())
    ...
    def load_ply(self, path):
        ...
        self._xyz = nn.Parameter(torch.tensor(xyz, dtype=torch.float, device=get_device()).requires_grad_(True))
        self._features_dc = nn.Parameter(torch.tensor(features_dc, dtype=torch.float, device=get_device()).transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(torch.tensor(features_extra, dtype=torch.float, device=get_device()).transpose(1, 2).contiguous().requires_grad_(True))
        self._opacity = nn.Parameter(torch.tensor(opacities, dtype=torch.float, device=get_device()).requires_grad_(True))
        self._scaling = nn.Parameter(torch.tensor(scales, dtype=torch.float, device=get_device()).requires_grad_(True))
        self._rotation = nn.Parameter(torch.tensor(rots, dtype=torch.float, device=get_device()).requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        # Prune labels to match pruned points
        if self.label.shape[0] > 0:
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_labels=None):
        d = {"xyz": new_xyz,
        ...
        # Concatenate labels for new points
        if new_labels is not None:
            self.label = torch.cat([self.label, new_labels], dim=0)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        padded_grad = torch.zeros((n_init_points), device=get_device())
        ...
        means =torch.zeros((stds.size(0), 3),device=get_device())
        ...
        # Propagate labels from parent points
        new_labels = None
        if self.label.shape[0] > 0:
            new_labels = self.label[selected_pts_mask].repeat(N)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_labels)

        prune_filter = torch.cat((selected_pts_mask, torch.zeros(N * selected_pts_mask.sum(), device=get_device(), dtype=bool)))
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_labels = None
        if self.label.shape[0] > 0:
            new_labels = self.label[selected_pts_mask]

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_labels)
