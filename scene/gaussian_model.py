...
# Try to import distCUDA2, fallback to Python implementation if not available
try:
...
def compute_dist2_fallback(points, device='cpu'):
    """
    ...
    # Shape: (N, N)
    diff = Points_tensor.unsqueeze(1) - Points_tensor.unsqueeze(0)  # (N, 1, 3) - (1, N, 3) = (N, N, 3)
    distances_sq = torch.sum(diff ** 2, dim=2)  # (N, N)
    
    # Get 4-th smallest distance (excluding self at 0 and taking 3-NN)
    # Sort distances and get index 3 (0-indexed: self, 1st NN, 2nd NN, 3rd NN)
    sorted_dists, _ = torch.sort(distances_sq, dim=1)
    dist2 = sorted_dists[:, 3]  # Take 3rd nearest neighbor distance
    
    return torch.clamp_min(dist2, 0.0000001)
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        label_source = getattr(pcd, "labels", None)
        if label_source is None:
            label_source = getattr(pcd, "label", None)
        if label_source is not None:
            label_values = np.asarray(label_source, dtype=np.int32).reshape(-1)
            if label_values.shape[0] != fused_color.shape[0]:
                label_values = np.zeros((fused_color.shape[0],), dtype=np.int32)
        else:
            label_values = np.zeros((fused_color.shape[0],), dtype=np.int32)
        self.label = torch.tensor(label_values, dtype=torch.int32, device=device)

        print("Number of points at initialisation : ", fused_point_cloud.shape[0])

        # Compute dist2 using available method
        if HAS_SIMPLE_KNN:
            dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().to(device)), 0.0000001)
        else:
            dist2 = compute_dist2_fallback(np.asarray(pcd.points), device=device)
    ...
    def save_ply(self, filepath):
        ...
        labels = None
        if self.label.numel() == xyz.shape[0]:
            labels = self.label.detach().cpu().numpy().astype(np.int32).reshape(-1)
        ...
        if labels is not None:
            elements[:] = [tuple(row) + (int(label),) for row, label in zip(attributes, labels)]
        else:
    ...
    def load_ply(self, path):
        ...
        if "label" in plydata.elements[0].data.dtype.names:
            labels = np.asarray(plydata.elements[0]["label"], dtype=np.int32).reshape(-1)
        else:
        ...
        self.label = torch.tensor(labels, dtype=torch.int32, device=device)
