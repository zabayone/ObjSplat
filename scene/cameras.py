...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        try:
            ...
            print(e)
            print(f"[Warning] Custom device {data_device} failed, fallback to default device" )
            self.data_device = get_device()
...
class MiniCam2:
    def __init__(self, pose, width, height, fovx, fovy):
        ...
        device = get_device()
        self.world_view_transform = torch.tensor(w2c.astype(np.float32)).transpose(0, 1).to(device)
        ...
        self.camera_center = -torch.tensor(pose[:3, 3], dtype=torch.float32).to(device)
...
class MiniCam_4d:
    def __init__(self, pose, width, height, fovx, fovy, time):
        ...
        self.znear, self.zfar = 0.01, 100
        device = get_device()
        self.world_view_transform = torch.tensor(getWorld2View2(R, T, np.array([0.0, 0.0, 0.0]), 1.0).astype(np.float32)).transpose(0, 1).to(device)
