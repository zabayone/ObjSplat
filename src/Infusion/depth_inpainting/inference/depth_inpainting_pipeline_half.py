...
class DepthEstimationInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        rgb_norm = rgb / 255.0
        rgb_norm = torch.from_numpy(rgb_norm).to(self.model_dtype)
        rgb_norm = rgb_norm.to(device)

        rgb_norm = rgb_norm.to(self.model_dtype)
    ...
    def __encode_empty_text(self):
        ...
        self.empty_text_embed = self.text_encoder(text_input_ids)[0].to(self.model_dtype) #[1,2,1024]
    ...
    def single_infer(self,input_rgb:torch.Tensor,
        ...
        h=input_rgb.shape[2]
        w = input_rgb.shape[3]
        mask = torch.from_numpy(cv2.resize(mask,(w,h))).float().to(device)
       
        zero_indices = torch.nonzero(mask== 0)
        disparity = torch.from_numpy(depth_numpy).float().to(device)
        ...
        mask_disparity = left_disparity_resized_normalized.repeat(1,3,1,1).to(self.model_dtype)
        mask_disparity = self.encode_depth(mask_disparity)
        mask = torch.nn.functional.interpolate(mask.unsqueeze(0).unsqueeze(0), size=(mask.shape[0] // 8, mask.shape[1] // 8)).to(self.model_dtype)
        mask_blend=mask
        ...
        disp_latents = self.encode_depth(depth_latents_no_mask.to(self.model_dtype))
        noise = torch.randn_like(disp_latents)
        depth_latent = self.scheduler.add_noise(disp_latents, noise, timesteps_add[:1])
        depth_latent = depth_latent.to(self.model_dtype)
        ...
        for i, t in iterable:
            ...
            depth_latent = self.scheduler.step(noise_pred, t, depth_latent).prev_sample.to(self.model_dtype)
            if blend:
                if i < len(timesteps) - 1:
                    noise_timestep = timesteps_add[i + 1]
                    disp_latent_step = self.scheduler.add_noise(
                        disp_latents, noise, torch.tensor([noise_timestep], device=device)
                    )
                    init_mask = F.interpolate(
                        ...
                        mode="bilinear",
                        align_corners=False,
                    ).repeat(1, 4, 1, 1).to(device).float()
    ...
    def encode_RGB(self, rgb_in: torch.Tensor) -> torch.Tensor:
        ...
        """
        rgb_in = rgb_in.to(self.model_dtype)
    ...
    def encode_depth(self, depth_latent: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            depth_latent (`torch.Tensor`):
                Depth input to encode.

        Returns:
            `torch.Tensor`: Encoded depth latent.
        """
        depth_latent = depth_latent.to(self.model_dtype)
